...
export function SalesChart({ data, compare }: { data: AnalyticsTimePoint[]; compare: boolean }) {
  return (
    <section className="admin-card min-w-0">
      <header className="border-b px-4 py-3.5 sm:px-5">
        <h2 className="text-[15px] font-semibold">Sales over time</h2>
        <p className="text-[11px] text-[var(--color-text-muted)]">
          Gross order revenue for the selected period
        </p>
      </header>
      <div
        className="h-[280px] px-2 pb-3 pt-4 sm:h-[320px] sm:px-4"
        role="img"
          ...
          <AreaChart data={data} margin={{ top: 6, right: 12, left: -6, bottom: 0 }}>
            <defs>
              <linearGradient id="analyticsSalesFill" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#29845a" stopOpacity={0.15} />
