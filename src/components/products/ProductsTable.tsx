      ...
      <table className="w-full min-w-[900px] border-collapse text-left text-[13px]">
        ...
        <tbody className="divide-y">
          {products.map((product) => (
            <tr
              key={product.id}
              className={`group transition-colors hover:bg-[var(--color-surface-subdued)] ${selected.has(product.id) ? "bg-[#f1f5f3]" : "bg-[var(--color-surface)]"}`}
            >
              ...
              <td className="px-2 py-2.5">
                <Link href={`/products/${product.id}`} className="flex items-center gap-3">
                  ...
                  <span className="min-w-0">
                    <span className="block truncate font-semibold text-[var(--color-text)] underline-offset-2 group-hover:underline">
                      {product.name}
                    </span>
